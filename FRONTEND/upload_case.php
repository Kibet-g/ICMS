...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($occurenceDateTime > $currentDateTime || $occurenceDateTime < $minDateTime) {
        // Display error message using JavaScript alert
        echo "<script>";
        echo "alert('The occurrence date and time must be within the last week.');";
        echo "window.location.href = 'upload_case.php';";
        echo "</script>";
        exit; // Stop further execution
    }
...
<head>
    ...
    <script>
        window.onload = function() {
            ...
            var oneWeekAgo = new Date(currentDateTime);
            oneWeekAgo.setDate(oneWeekAgo.getDate() - 7);
            var oneWeekAgoDate = oneWeekAgo.toISOString().split('T')[0];

            // Set min and max attributes for date input
            document.getElementsByName("occurence_date")[0].setAttribute('min', oneWeekAgoDate);
            document.getElementsByName("occurence_date")[0].setAttribute('max', currentDate);

            // Set min attribute for time input if the date is today
    ...
    <div class="header">
        <div class="report-form">
            ...
            <form method="POST" enctype="multipart/form-data" action="upload_case.php">
                ...
                Date of Occurrence: <input type="date" name="occurence_date" required><br><br>
                Time of Occurrence: <input type="time" name="occurence_time" required><br><br>
